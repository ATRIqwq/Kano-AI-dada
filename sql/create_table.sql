...
create table if not exists user_answer_01
(
...
(
    ...
    index idx_appId (appId),
    index idx_userId (userId)
    ) comment '用户答题记录_1' collate = utf8mb4_unicode_ci;

create table if not exists user_answer_2
(
    id              bigint auto_increment primary key,
    appId           bigint                             not null comment '应用 id',
    appType         tinyint  default 0                 not null comment '应用类型（0-得分类，1-角色测评类）',
    ...
    index idx_appId (appId),
    index idx_userId (userId)
    ) comment '用户答题记录_2' collate = utf8mb4_unicode_ci;